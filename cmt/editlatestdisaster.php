...
{
    $log = $_GET['id'];
    $a = "SELECT * FROM tb1_disaster WHERE disaster_id='$log'";
...
{	
    if($_FILES["image"]["name"] != "")
    {
        ...
        $updated3 = mysqli_query($con,"UPDATE tb1_disaster SET name='$name',description='$description',occurred_on='$date',image='$location' WHERE disaster_id='$log'");	
        if($updated3)
        {
            echo ("<script LANGUAGE='JavaScript'>window.alert('Succesfully Updated');window.location.href='managelatestdisaster.php?flag=1';</script>");	
        }
        else
        {
    ...
    else
    {
        ...
        $description = $_POST['description'];
        $date = $_POST['dob'];
        $updated = mysqli_query($con,"UPDATE tb1_disaster SET name='$name',description='$description',occurred_on='$date' WHERE disaster_id='$log'");	    
        if($updated)
        {
            echo ("<script LANGUAGE='JavaScript'>window.alert('Succesfully Updated');window.location.href='managelatestdisaster.php?flag=1';</script>");	
        }
