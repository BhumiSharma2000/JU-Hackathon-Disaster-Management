<?php
    ...
    { 
        $log = $_GET['id'];
        $a = "SELECT * FROM tb1_login WHERE login_id='$log'";
        $aa = mysqli_query($con,$a);
        $value = mysqli_fetch_array($aa);
        $id1 = $value['login_id'];
        $b = "SELECT * FROM tb1_detail WHERE login_id='$id1'";
    ...
    <?php
        ...
        { 
            if($_FILES["image"]["name"] != "")
            {
                ...
                $updated = mysqli_query($con,"UPDATE tb1_login SET email_id='$email' WHERE login_id='$log'");
                $updated2 = mysqli_query($con,"UPDATE tb1_detail SET name='$name',profile_pic='$location',address='$address',dob='$dob' WHERE login_id='$id'");
                $updated3 = mysqli_query($con,"UPDATE tb1_login SET phone_no='$newphone' WHERE login_id='$log'");
                if($updated && $updated2)
                {
                    if($updated3)
                    {
                        echo ("<script LANGUAGE='JavaScript'>window.alert('Succesfully Updated');window.location.href='manageusers.php?flag=1';</script>");           
                    }
                    else
                    {
            ...
            else
            {
                  ...
                  $dob = $_POST['dob'];
                  $email = $_POST['email'];
                  $updated = mysqli_query($con,"UPDATE tb1_login SET email_id='$email',phone_no='$newphone' WHERE login_id='$id'");
                  $updated2 = mysqli_query($con,"UPDATE tb1_detail SET name='$name',address='$address',dob='$dob' WHERE login_id='$id'");
                  if($updated && $updated2)
                  {
                        echo ("<script LANGUAGE='JavaScript'>window.alert('Succesfully Profile Updated');window.location.href='manageusers.php?flag=1';</script>");          
                  }      
                  else
                  {
                      echo "<font style='color:red'>Error...</font>";
